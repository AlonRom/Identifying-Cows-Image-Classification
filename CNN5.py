...
class CNN5(nn.Module):
    ...
    # F.relu
    def forward(self, x):
        x = F.normalize(F.max_pool2d(self.activation_func(self.conv1(x)), (2, 2)))
        x = F.dropout(F.normalize(F.max_pool2d(self.activation_func(self.conv2(x)), 2)))
        x = x.view(-1, self.num_flat_features(x))
        x = F.dropout(F.normalize(self.activation_func(self.fc1(x))))
